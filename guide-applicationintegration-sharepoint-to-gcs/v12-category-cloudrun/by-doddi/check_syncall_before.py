...
def run_category_check(category, params, token, headers, gcs_cache):
    ...
    for s_path in site_list:
                ...
                while q:
                    ...
                    for item in items:
                        if "folder" in item and item.get("folder", {}).get("childCount", 0) > 0:
                            ...
                            gcs_key_full = f"{category_prefix}/files/{curr_path}/{f_name}" if category_prefix else f"files/{curr_path}/{f_name}"
                            gcs_key_full = gcs_key_full.replace("//", "/")
                            
                            gcs_key_short = f"{category_prefix}/files/{f_name}" if category_prefix else f"files/{f_name}"
                            gcs_key_short = gcs_key_short.replace("//", "/")
                            
                            item_info = {"id": item["id"], "Name": f_name, "Subsite": sname, "IsPage": False, "Path": gcs_key_full}
                            items_found.append(item_info)
                            
                            cached_time_full = gcs_cache.get(gcs_key_full)
                            cached_time_short = gcs_cache.get(gcs_key_short)
                            cached_time = cached_time_full or cached_time_short
        ...
        def scan_subsite_pages_unthrottled(st):
            items_found = []
            sync_found = []
            s_id = st["id"]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
            ...
            s_futures = [pool.submit(scan_subsite_pages_unthrottled, st) for st in target_sites]
            seen_page_paths = set()
                ...
                try:
                    ...
                    for item in res_all:
                        if item.get("IsPage"):
                            ...
                            cat_items.append(item)
                    for item in res_sync:
                        if item.get("IsPage"):
                            if not any(x.get("Path") == item["Path"] for x in cat_sync):
                                cat_sync.append(item)
                        else:
                            cat_sync.append(item)
                except Exception:
                    pass

                ...
                while q:
                    ...
                    for item in items:
                        if "folder" in item and item.get("folder", {}).get("childCount", 0) > 0:
                            ...
                            item_info = {"id": item["id"], "Name": f_name, "Subsite": sname, "Path": gcs_key_full}
                            items_found.append(item_info)
                            if "/" not in curr_path:
                                if gcs_key_full not in gcs_cache and gcs_key_short not in gcs_cache:
                                    missing_found.append(item_info)
                            else:
                                if gcs_key_full not in gcs_cache:
                                    missing_found.append(item_info)
            except Exception:
                pass

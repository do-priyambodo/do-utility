...
def list_drive_items_recursive(token, drive_id, item_id="root", parent_path="", all_results=None, sync_results=None, base_file_url="", bucket_obj=None):
    ...
    for item in items:
        ...
        if "folder" in item:
            ...
            list_drive_items_recursive(token, drive_id, item_id, new_parent_path, all_results, sync_results, base_file_url, bucket_obj)
        else:
            # It is a file
...
def main(request):
    ...
    if bucket_name and not force_full_sync:
        try:
            ...
            print(f"Warning: Could not init GCS bucket client for incremental check: {e}")
    ...
    try:
        ...
        # Fallback to first document library drive if requested library name not matched
        ...
        if target_drive_id:
            ...
            list_drive_items_recursive(token, target_drive_id, "root", "", all_list, sync_list, base_file_url, bucket_obj)
        ...
        if pages_resp.status_code == 200:
            ...
            for p in pages:
                ...
                rel_page_path = f"pages/{html_name}"
                ...
                needs_sync = True
                # Option 1: Incremental sync check for site pages
                if bucket_obj:
                    try:

...
The validator script [validate_params.py](file:///usr/local/google/home/priyambodo/Coding/DO-PRIYAMBODO/customer-maxis/do-applicationintegration/app/v2.0/util/validate_params.py) is a diagnostic tool designed to verify your configuration file `parameters.json` before deploying any Cloud Functions or Application Integration workflows.
...
* **Creation Step**:
  ...
  3. Enter a name (e.g., `doddi-sa-sharepoint-gcs`) and click **Create and Continue**.
  4. (Optional) Assign temporary roles or configure them later using `check_permissions.py` recommendations.
  5. Click **Done**.
  6. Copy the fully qualified email address: `doddi-sa-sharepoint-gcs@<project-id>.iam.gserviceaccount.com`.

  #### Option 2: Using Command Line Interface (CLI)
  Run the following `gcloud` command in your terminal:
  ```bash
  gcloud iam service-accounts create "doddi-sa-sharepoint-gcs" \
      --description="Identity used to execute the Cloud Function and run the integrations" \
      --display-name="doddi-sa-sharepoint-gcs" \
      --project="<project-id>"
  ```
  The fully qualified service account email will be: `doddi-sa-sharepoint-gcs@<project-id>.iam.gserviceaccount.com`.
...
* **Creation Step**:
  ...
  2. Click **Create Secret**. Name it (e.g., `doddi-secret-sharepoint-clientsecret`).
  3. Paste the Azure Client Secret value as the secret value and click **Create**.
  4. Copy the resource ID path of version 1:
     `projects/<project-number>/secrets/doddi-secret-sharepoint-clientsecret/versions/1`

  #### Option 2: Using Command Line Interface (CLI)
  Run the following `gcloud` commands in your terminal:
  ```bash
  # 1. Create the Secret Manager container
  gcloud secrets create "doddi-secret-sharepoint-clientsecret" \
      --replication-policy="automatic" \
      --project="<project-id>"

  # 2. Add the M365 client secret value as version 1
  echo -n "YOUR_AZURE_CLIENT_SECRET_VALUE" | gcloud secrets versions add "doddi-secret-sharepoint-clientsecret" \
      --data-file=- \
      --project="<project-id>"
  ```
  The secret resource path will be: `projects/<project-number>/secrets/doddi-secret-sharepoint-clientsecret/versions/1`.
...
* **Creation Step**:
  ...
  2. Click **Create**.
  3. Set a globally unique name (e.g., `doddi-bucket-sharepoint-sync`).
  ...
  Run the following `gcloud` command in your terminal:
  ```bash
  gcloud storage buckets create gs://doddi-bucket-sharepoint-sync \
      --location="<location>" \
      --project="<project-id>"
  ```
...
* **Creation Step**:
  ...
  4. **Connection Name**: `doddi-connection-gcs-sync`.
  ...
  8. Path: `projects/<project-id>/locations/<location>/connections/doddi-connection-gcs-sync`.
...
* **Creation Step**:
  ...
  4. **Connection Name**: `doddi-connection-sharepoint-sync`.
  5. **Location**: Same as `CONFIG_Location`.
  ...
  10. Path: `projects/<project-id>/locations/<location>/connections/doddi-connection-sharepoint-sync`.

### 9. `CONFIG_SharePoint_Hostname` & `CONFIG_Sharepoint_Sites`
* **Goal**: SharePoint domain and subsite target.
* **Creation Step**:
  1. Open your SharePoint Online site.
  2. Hostname is the main domain (e.g., `priyambodo.sharepoint.com`).
  3. Subsite path is the subsite folder name (e.g., `sites/doddi-sharepoint-to-gcs`). Create this subsite inside your SharePoint Admin dashboard if it does not exist yet. Ensure it contains a document library named `Shared Documents`.

### 10. `CONFIG_CloudFunction_Name`
* **Goal**: Name of the deployed traversal Cloud Function.
* **Creation Step**:
  * Set this string parameter (e.g., `doddi-sharepoint-list-files`). Running `./deploy_cf.sh` will deploy it.
...
* **Creation Step**:
  * Set these strings (e.g., `doddi-sharepoint-gcs-parent` and `doddi-sharepoint-gcs-child`). Running `deploy_workflows.py` will deploy and link them.

...
3. **Strict Runtime Schema Enforcement (`cf-sharepoint/config_schema.py`)**:
   * When deployed to Cloud Run, `validate_parameters()` automatically runs at startup to verify all required configuration keys (`CONFIG_ProjectId`, `CONFIG_GCS_Bucket`, `CONFIG_M365_Tenant_Id`) before executing network operations.
   * Fails fast with clear actionable error guidance if any key is missing or malformed.

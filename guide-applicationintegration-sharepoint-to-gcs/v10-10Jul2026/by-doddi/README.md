...
### Step 3.5: Pre-Flight Interactive Diagnostics & One-Chance Enterprise Verification

Before deploying integration workflows or executing in a customer's production environment (**where you have only 1 attempt**), run our **One-Chance Enterprise Production Readiness Checklist** to verify hardware sizing, timeout limits, Entra ID authentication, and inventory counts:

#### A. Enterprise Hardware & Timeout Verification (`Cloud Run Sizing`)
Verify that the Traversal Cloud Function (`july1st-sharepoint-list-files` or `$CONFIG_CloudFunction_Name`) is configured with 8 GiB memory (`8192Mi`) and maximum request timeout (`900s` / 15 minutes) to run Playwright Chromium with maximum safety margin:
```bash
# 1. Inspect deployed Cloud Run service limits
...
```
* **Recommended Output**: Memory should be `8Gi` (`8192Mi`), and `timeoutSeconds` must be `900`.
* **To configure 8 GB Memory (`8192Mi`) for Maximum Safety**:
  ```bash
...
#### B. Authentication & Pre-Flight Diagnostic Scripts

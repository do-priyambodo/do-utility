...
Before executing any data movement, verify that your local configuration, unit test suite, and cloud authentication bindings are healthy.

### Step 0.0: Run Automated Unit & Schema Verification Harness (`Rugged Enterprise Test Suite`)
Execute the automated unit test suite (`tests/test_config_schema.py` and `tests/test_discovery.py`). This verifies that the strict `config_schema.py` validator and unified `sharepoint_engine/discovery.py` classification engine pass all deterministic checks in milliseconds without requiring live cloud credentials:
```bash
python3 -m unittest discover tests -v
```
*Expected Output:* `Ran 5 tests in 0.001s -> OK`

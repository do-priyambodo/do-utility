...
- **DO NOT** perform direct feature edits or start tasks inside `do-utility`. Always work strictly inside `customer-maxis/.../v13-final` and sync/mirror changes downstream to `do-utility`.

## Mandatory Bidirectional Code Mirroring & Auto-Push Rule
For all modifications performed:
Whenever any file is modified, added, or deleted inside the following active version directory (`v13-final`):
- **Location A (`customer-maxis` - Primary):** `/usr/local/google/home/priyambodo/Coding/DO-PRIYAMBODO/do-CUSTOMERS/customer-maxis/do-applicationintegration/app/v13-final`
- **Location B (`do-utility` - Mirror Target):** `/usr/local/google/home/priyambodo/Coding/DO-PRIYAMBODO/do-utility/guide-applicationintegration-sharepoint-to-gcs/v13-final`
...
- Always use **relative paths** (`./deploy/deploy_cloud_run.sh`, `DO-SYNC-ALL-SHAREPOINT.md`) or generic placeholder directories (`/path/to/your/repo/v13-final/by-yourorg`).

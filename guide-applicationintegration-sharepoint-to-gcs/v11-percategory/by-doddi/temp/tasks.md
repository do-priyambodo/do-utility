...
- [x] **Task 1.1:** Create `config/config-category.json` with top-level `"root_portal_site"` (`sites/DEN`) and 3-Tier Sharding Matrix (`tier1-den-root-only`, `tier1-business`, `tier1-consumer`, `tier1-hotlink`, `tier1-system-procedure`, `tier2-medium-departments`, `tier3-specialized-teams`), standardizing on `"sharepoint_library": "all"`.
- [x] **Task 1.2:** Clean up `config-parameters.json` by removing target scopes (`CONFIG_Sharepoint_Sites` and `CONFIG_Sharepoint_Library`), preserving only cloud infrastructure variables, Secret Manager path, and `CONFIG_SharePoint_Hostname`.
- [x] **Task 1.3:** Create/update configuration loader logic (`util/config_loader.py` or helper in `main.py`) to seamlessly read both `config-parameters.json` and `config-category.json` (from local disk or GCS bucket).
- [x] **Task 1.4:** Run JSON validation (`python3 -m json.tool config/config-category.json`) and confirm clean syntax across both repositories.

---

### Phase 2: Diagnostic & Fast Discovery Engine (`check/` utilities)
- [x] **Task 2.1:** Create `check/discover_categories.py` — 2-second fast discovery tool that connects via Graph API (`GET /v1.0/sites/{id}/subsites`), reads `root_portal_site` from `config-category.json` (or `--root=...`), and outputs all child departments without counting items.
- [x] **Task 2.2:** Update `check/check_syncall_before.py` to load `config-category.json` and support both **Mode A** (`--category=tier1-business` targeted 15s audit) and **Mode B** (Sequential category-by-category loop with RAM wiping and summary table).
...
- [x] **Task 3.1:** Update `cf-sharepoint/main.py` entry point to execute Option 1 Master Category Loop across `categories[]` from `config-category.json`, plus supporting optional single-category on-demand overrides via `--update-env-vars="TARGET_CATEGORY_ID=..."`.
...
- [x] **Task 4.1:** Update `deploy/deploy_cloud_run.sh` to deploy the V11 container without hardcoded SharePoint site variables, setting `CONFIG_SITES_SYNC_PATH=config/config-category.json`.
...
- [x] **Task 4.3:** Create `DO-SYNC-CATEGORY.md` comprehensive operator runbook detailing `discover_categories.py`, Option 1 master loop, single-category overrides (`--update-env-vars`), and diagnostic verification.

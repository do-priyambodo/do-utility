...
# and verifies that config-config-parameters.json is valid before Cloud Run deployment.
set -euo pipefail
...
echo -e "\n⚙️  3. Verifying local configuration (config-config-parameters.json)..."
if [[ ! -f "config-config-parameters.json" ]]; then
    echo "❌ Error: config-config-parameters.json not found in current directory!"
    exit 1
fi

PROJECT_ID=$(python3 -c 'import json; print(json.load(open("config-config-parameters.json")).get("CONFIG_ProjectId", ""))' 2>/dev/null || echo "")
BUCKET_NAME=$(python3 -c 'import json; print(json.load(open("config-config-parameters.json")).get("CONFIG_GCS_Bucket", ""))' 2>/dev/null || echo "")
JOB_NAME=$(python3 -c 'import json; print(json.load(open("config-config-parameters.json")).get("CONFIG_CloudFunction_Name", "yourorg-sharepoint-list-files"))' 2>/dev/null || echo "")
PARENT_INT=$(python3 -c 'import json; print(json.load(open("config-config-parameters.json")).get("CONFIG_Parent_Integration_Name", ""))' 2>/dev/null || echo "")

if [[ -z "${PROJECT_ID}" || -z "${BUCKET_NAME}" ]]; then
    echo "❌ Error: CONFIG_ProjectId or CONFIG_GCS_Bucket is missing in config-config-parameters.json!"
    exit 1
fi

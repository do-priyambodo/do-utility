    ...
    if action == "merge_metadata":
        ...
        try:
            ...
            if combined_records:
                ...
                return (json.dumps({"status": "success", "consolidated_records": len(combined_records)}), 200, {"Content-Type": "application/json"})
            else:
                print("⚠️ Metadata Merger Warning: No sharded manifests found to merge.")
                return (json.dumps({"status": "warning", "message": "No shard manifests found"}), 200, {"Content-Type": "application/json"})

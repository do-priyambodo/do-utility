...
CATEGORIES_COUNT=$(python3 -c "import json; print(len(json.load(open('parameters.json')).get('CONFIG_Categories', [])))")
TASKS_COUNT=$((CATEGORIES_COUNT + 1))
echo "🚀 Creating or updating Cloud Run Job (${SERVICE_NAME}) with image ${IMAGE_NAME} (Tasks: ${TASKS_COUNT})..."
